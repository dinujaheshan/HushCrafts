import { db } from '../../apps/backend/functions/src/config/firebaseAdmin';
...
async function seedDatabase() {
  ...
  for (const prod of productsList) {
    ...
    for (const v of variants) {
      ...
      await db.collection('products').doc(prod.id).collection('variants').doc(v.id).set({
        id: v.id,
        sku: v.sku,
        price: null,
        compareAtPrice: null,
      ...
      await db.collection('inventory').doc(v.sku).set({
        id: v.sku,
        productId: prod.id,
        variantId: v.id,
        quantity: v.stock,
  ...
  await db.collection('settings').doc('store_settings').set({
    values: {
